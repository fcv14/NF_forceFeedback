...
public class EditUI : MonoBehaviour
{
    ...
    public Button Btn_Return2MainUIScene;
    ...
    [Space(10)]
    public GameObject Obj_TaskEditSuccessfully;
    public Text Txt_TaskEditScuccessfully;
    public Button Btn_Okay_TaskEditSuccessfully;
    ...
    void Start()
    {
        ...
        Btn_Okay_TaskEditSuccessfully.onClick.AddListener(delegate { OkayBtn_TaskEditSuc(); });
        Btn_Return2MainUIScene.onClick.AddListener(delegate { Return2MainUI(); });

        ColorInit();
    }
    void Make_Hole()
    {
        if (EditTask.List_selectedCubes.Count != 0)
        {
            //換色
            ...
            //修正parent的初始pos
            foreach (var selectedCube in EditTask.List_selectedCubes)
            {
                parent_z_pos += selectedCube.transform.position.z;
            }
            parent_z_pos /= EditTask.List_selectedCubes.Count;
            ...
            foreach (var selectedCube in EditTask.List_selectedCubes)
            {
                ...
                EditTask.List_triggers.Add(trg);
        ...
        EditTask.List_selectedCubes.Clear();
        //triggers_List.Clear();
        parent_z_pos = 0;
        //還原
    }
    ...
    void Slider_alphaChange()
    {
        ...
        foreach (var trg_P in EditTask.List_triggers)
        {
            Color C_A = new Color();
            C_A.a = Cube_alpha;
    ...
    void ConfirmBtn_clicked()
    {
        ...
        {
            TaskSave();
            Txt_TaskEditScuccessfully.text = "Task建立完成!";
            Obj_TaskEditSuccessfully.SetActive(true);
            //saveWork
        }
    }
    ...
    void CubesData()
    {
        ...
        for (int i = 0; i < EditTask.List_wallCubes.Count; i++)
        {
            OriginCubeData originCubeData = new OriginCubeData();

            originCubeData.originCubeName = EditTask.List_wallCubes[i].name;

            originCubeData.originCubePos = new float[3];
            originCubeData.originCubePos[0] = EditTask.List_wallCubes[i].transform.position.x;
            originCubeData.originCubePos[1] = EditTask.List_wallCubes[i].transform.position.y;
            originCubeData.originCubePos[2] = EditTask.List_wallCubes[i].transform.position.z;

            originCubeData.originCubeColorRGBA = new float[4];
            originCubeData.originCubeColorRGBA[0] = EditTask.List_wallCubes[i].transform.GetComponent<Renderer>().material.color.r;
            originCubeData.originCubeColorRGBA[1] = EditTask.List_wallCubes[i].transform.GetComponent<Renderer>().material.color.g;
            originCubeData.originCubeColorRGBA[2] = EditTask.List_wallCubes[i].transform.GetComponent<Renderer>().material.color.b;
            originCubeData.originCubeColorRGBA[3] = EditTask.List_wallCubes[i].transform.GetComponent<Renderer>().material.color.a;
    ...
    void OkayBtn_TaskEditSuc()
    {
        IPF_TaskName.text = "";
        Obj_TaskEditSuccessfully.SetActive(false);
    }
